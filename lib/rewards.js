...
async function deleteOwnedSubmission(id, ownerToken) {
  ...
  // Pending entries may always be withdrawn by the participant.
  // This also supports older Pending submissions created before
  // owner_token was introduced.
  if (String(item.status || "pending").toLowerCase() === "pending") {
    return deleteSubmission(cleanId);
  }

  // For non-Pending entries, keep ownership verification enabled.
