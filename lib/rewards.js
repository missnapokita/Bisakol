...
async function updateOwnedSubmission(id, ownerToken, body) {
  const cleanId = cleanText(id, 120);
  const token = cleanText(ownerToken, 120);
  const item = await getSubmission(cleanId);

  if (!item || !token || item.owner_token !== token) {
    const err = new Error("Submission ownership could not be verified");
    err.statusCode = 403;
    throw err;
  }

  if (item.status === "approved" || item.is_winner === true) {
    const err = new Error("Approved or winning entries cannot be edited");
    err.statusCode = 409;
    throw err;
  }
  ...
  return updateSubmission(cleanId, {
    tiktok_username: username,
    tiktok_url: url,
    content_type: contentType,
    reward_choice: rewardChoice,
...
async function deleteOwnedSubmission(id, ownerToken) {
  const cleanId = cleanText(id, 120);
  const token = cleanText(ownerToken, 120);
  const item = await getSubmission(cleanId);
  ...
  if (item.is_winner === true) {
    const err = new Error("Winning entries cannot be deleted by the participant");
    err.statusCode = 409;
    throw err;
  }
  ...
  // Legacy Pending entries were created before owner_token existed.
  // They can still be withdrawn by the same local My Submissions UI.
  if (status === "pending" && !item.owner_token) {
    return deleteSubmission(cleanId);
  }

  // Pending, Approved, or Rejected entries can be deleted any time
  // when the current device owns the submission.
  if (!token || item.owner_token !== token) {
    const err = new Error("Submission ownership could not be verified");
    err.statusCode = 403;
    throw err;
  }
...
async function setEventConfig(config) {
  const next = {
    ...
    start_at_iso: Math.max(0, Number(config.start_at || 0) || 0) > 0
      ? new Date(Math.max(0, Number(config.start_at || 0) || 0)).toISOString()
      : "",
    updated_at: Date.now()
  };
